...
def load_model():
    if not Path("kb-bert-cased-ner-optimized-quantized.onnx").is_file():
        request.urlretrieve("https://www.dropbox.com/s/bjr14jw6n2o3dmu/kb-bert-cased-ner-optimized-quantized.onnx?dl=1", "kb-bert-cased-ner-optimized-quantized.onnx")
    onnx_options = SessionOptions()
    session = InferenceSession("kb-bert-cased-ner-optimized-quantized.onnx", onnx_options, providers=["CPUExecutionProvider"])
    tokenizer = AutoTokenizer.from_pretrained("KB/bert-base-swedish-cased-ner", use_fast=False)
    return session, tokenizer

@st.cache(allow_output_mutation=True, hash_funcs={InferenceSession: lambda _: None, BertTokenizer: lambda _: None})
...
model = st.radio("Select which model to use", ('Flair (F1: 85.6 - faster & 80MB)', 'Bert (F1: 92.0 - 120MB)'))
